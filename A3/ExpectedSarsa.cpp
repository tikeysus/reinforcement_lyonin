...
ExpectedSarsa::ExpectedSarsa(const Gridworld& env, const TrainingOptions& options)
    : env_(env), options_(options) {}

TrainingSummary ExpectedSarsa::run() {
...
SingleRunResult ExpectedSarsa::runSingle(const uint64_t seed) const {
    mt19937_64 rng(seed);
    ...
    for (int episode = 0; episode < options_.episodes; ++episode) {
        ...
        while (steps < options_.maxStepsPerEpisode && !env_.isTerminal(state)) {
            ...
            const double expectedNext = env_.isTerminal(nextState)
                ? 0.0
                : expectedEpsilonGreedyValue(result.qValues[nextState], epsilon);
            const double tdTarget = reward + options_.gamma * expectedNext;
