...
Sarsa::Sarsa(const Gridworld& env, const TrainingOptions& options)
    : env_(env), options_(options) {}

TrainingSummary Sarsa::run() {
...
SingleRunResult Sarsa::runSingle(const uint64_t seed) const {
    mt19937_64 rng(seed);
    ...
    for (int episode = 0; episode < options_.episodes; ++episode) {
        ...
        int action = epsilonGreedyAction(result.qValues[state], epsilon, rng);
        ...
        while (steps < options_.maxStepsPerEpisode && !env_.isTerminal(state)) {
            ...
            if (env_.isTerminal(nextState)) {
                ...
                estimate += options_.alpha * (reward - estimate);
                break;
            }

            const int nextAction = epsilonGreedyAction(result.qValues[nextState], epsilon, rng);
            const double tdTarget = reward + options_.gamma * result.qValues[nextState][nextAction];
            double& estimate = result.qValues[state][action];
